...
namespace GoFishTests
{
    ...
    public class PlayerTests
    {
        ...
        public void TestRandomValueFromHand()
        {
            ...
            Assert.AreEqual("Ace", player.RandomValuesFromHand().ToString());

            Player.Random = new MockRandom() { ValueToReturn = 4 };
            Assert.AreEqual("Two", player.RandomValuesFromHand().ToString());

            Player.Random = new MockRandom() { ValueToReturn = 8 };
            Assert.AreEqual("Three", player.RandomValuesFromHand().ToString());
        }
    }
}